...
function formatString(format) {
  if (format === 'byte') return Joi.binary().encoding('base64');else if (format === 'binary') return Joi.binary();else if (format === 'email') return Joi.string().email();else if (format === 'uuid') return Joi.string().guid();else if (format === 'date') return Joi.date();else if (format === 'date-time') return Joi.date();else if (format === 'password') return Joi.string();else return Joi.string();
}

function formatInteger(joi, format) {
  if (format === 'int32') return joi.min(ranges.int32.min).max(ranges.int32.max);else if (format === 'int64') return joi.min(ranges.int64.min).max(ranges.int64.max);else return joi;
}

function formatNumber(joi, format) {
  if (format === 'float') return joi.min(ranges.float.min).max(ranges.float.max);else if (format === 'double') return joi.min(ranges.double.min).max(ranges.double.max);else return joi;
}

function initJoiWithType(type, format) {
  if (dataTypes.indexOf(type) == -1) return Joi.any();
...
function compose(field) {
  ...
  if (field.required) joi = joi.required();else joi = joi.optional();
