...
function getData(ctx, dataPos) {
  ...
  if (dataPos == 'header') data = ctx.headers;else if (dataPos == 'path') data = ctx.params;else if (dataPos == 'query') data = ctx.query;else if (dataPos == 'body') data = ctx.request.body;else if (dataPos == 'session') data = ctx.session;

  return data;
}

function validRequest(ctx, paramSchema) {
  const headerSchemas = _.filter(paramSchema, it => {
    return it.in === 'header';
  });
  const pathSchemas = _.filter(paramSchema, it => {
    return it.in === 'path';
  });
  const querySchemas = _.filter(paramSchema, it => {
    return it.in === 'query';
  });
  const bodySchemas = _.filter(paramSchema, it => {
    return it.in === 'body';
  });
  const sessionSchemas = _.filter(paramSchema, it => {
    return it.in === 'session';
  });
...
function parse(paramSchema) {
  ...
  return function swaggerRequest(ctx, next) {
    let error = validRequest(ctx, paramSchema);
